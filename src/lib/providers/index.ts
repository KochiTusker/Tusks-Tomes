...
let openrouterSingleton: OpenRouterProvider | null = null
...
function rebuild(keys: CloudKeys): void {
  ...
  openrouterSingleton = new OpenRouterProvider({ apiKey: keys.openrouter })
}
...
export async function refreshProviders(): Promise<void> {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (previous && next) {
      for (const slot of ['gemini', 'geminiFallback', 'claude', 'openai', 'openrouter'] as const) {
...
function openrouterProvider(): OpenRouterProvider {
  if (!openrouterSingleton) openrouterSingleton = new OpenRouterProvider()
...
export function getCloudProvider(
  name: 'gemini' | 'claude' | 'openai' | 'claudeCode' | 'codex' | 'openrouter',
  ...
  // Codex and OpenRouter must resolve BEFORE the openai fall-through — the
  // tail return is a fall-through, and letting either hit it would silently
  // route the user onto OpenAI API-key billing.
