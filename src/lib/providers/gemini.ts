  ...
  async generate(req: GenerateRequest, opts: GenerateOptions = {}): Promise<GenerateResponse> {
    ...
    while (attempt <= MAX_RETRIES) {
      ...
      try {
        ...
        // Thinking tokens are billed as output but arrive in their own field.
        // Prefer the explicit count; fall back to deriving it from the total,
        // since older SDK versions omit thoughtsTokenCount while still
        // including the thinking in totalTokenCount.
        const visibleOut = usageMeta?.candidatesTokenCount ?? 0
        const promptIn = usageMeta?.promptTokenCount ?? 0
        const explicitThoughts = (usageMeta as { thoughtsTokenCount?: number } | undefined)
          ?.thoughtsTokenCount
        const derivedThoughts =
          usageMeta?.totalTokenCount !== undefined
            ? Math.max(0, usageMeta.totalTokenCount - promptIn - visibleOut)
            : 0
        const thinkingTokens = explicitThoughts ?? derivedThoughts
        const usage: Usage = {
          inputTokens: promptIn,
          cachedInputTokens: usageMeta?.cachedContentTokenCount ?? undefined,
          // Report what is BILLED, not what is visible. Thinking is charged at
          // the output rate, and on the chronicle phase it can be several times
          // the size of the prose itself.
          outputTokens: visibleOut + thinkingTokens,
