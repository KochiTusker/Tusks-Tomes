...
describe('buildBudgetModePerPhase', () => {
  it('routes every routable phase to the fast model for the chosen provider', () => {
    const perPhase = buildBudgetModePerPhase('claudeCode', undefined)
    ...
    for (const phase of ['phase1', 'phase2', 'phase3', 'phase4', 'phase6'] as const) {
      ...
      if (entry?.target === 'cloud') {
        expect(entry.cloudProvider).toBe('claudeCode')
        expect(entry.modelId).toBe(BUDGET_FAST_MODELS.claudeCode)
      }
    }
  })
  ...
  it('uses gpt-5-mini for openai', () => {
    const perPhase = buildBudgetModePerPhase('codex', undefined)
...
describe('buildGeminiSmartBudgetPerPhase', () => {
  it('returns null for non-Gemini providers (preset is Gemini-only)', () => {
    expect(buildGeminiSmartBudgetPerPhase('claudeCode')).toBeNull()
    expect(buildGeminiSmartBudgetPerPhase('codex')).toBeNull()
  })
