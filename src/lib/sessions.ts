...
export async function listConfiguredCloudProviders(): Promise<CloudProvider[]> {
  ...
  return summary.configured.filter(
    (k): k is CloudProvider =>
      k === 'gemini' || k === 'claude' || k === 'openai' || k === 'openrouter'
  )
}
 ...
 */
export async function autoResolveSession(opts?: {
  /** Resolve without persisting `lastSelectedProvider`. Read-only surfaces
   *  (the phase rail preview) must pass true — previewing a run should
   *  never write routing.json. Run starts omit it so the choice sticks. */
  dryRun?: boolean
  ...
  if (options.length === 1) {
    ...
    return buildSession(only.provider, { geminiTier: only.geminiTier, dryRun })
  }
  const routing = await getRouting()
  const previous = optionFromRouting(options, routing.lastSelectedProvider, routing.geminiTier)
  if (previous) {
    return buildSession(previous.provider, { geminiTier: previous.geminiTier, dryRun })
  }
  ...
  return buildSession(first.provider, { geminiTier: first.geminiTier, dryRun })
}
