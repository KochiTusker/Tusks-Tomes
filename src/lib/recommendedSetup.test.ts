...
describe('Obsidian vault', () => {
  it('enables the add-on and records the chosen folder', () => {
    const plan = buildSetupPlan(base, { ...answers, vaultPath: 'D:/Vault' })
    expect(step(plan, 'addon-obsidian-vault-addon')?.skipped).toBe(false)
  ...
  it('requires a restart when the Obsidian add-on is newly enabled', () => {
    const plan = buildSetupPlan(base, { ...answers, vaultPath: 'D:/Vault' })
    ...
    const plan = buildSetupPlan(
      {
        ...
        routing: {
          ...
          perPhase: { phase3: { target: 'cloud', cloudProvider: 'claude', modelId: 'claude-opus-4-8' } },
        },
      },
      answers,
    )
...
describe('restart handling', () => {
  it('requires a restart when an add-on is newly enabled', () => {
    // Add-on routes mount once at startup (server/addons/loader.ts), so a
    // newly-enabled add-on cannot serve traffic in the running process.
    const plan = buildSetupPlan(
