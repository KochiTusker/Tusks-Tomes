...
const SIMPLE_GLOSSARY: GlossaryDocument = {
  version: 1,
  safeReplacements: [{ from: 'broady', to: 'Piers' }],
  contextualHints: [],
}
...
const RICHER_GLOSSARY: GlossaryDocument = {
  version: 1,
  safeReplacements: [
    { from: 'broady', to: 'Piers' },
    { from: 'kazle', to: 'Pernille' },
  ],
  contextualHints: [
    { canonical: 'Piers', commonMishears: ['Broady'], notes: 'Halfling rogue.' },
  ],
}
...
describe('computeRunFingerprint', () => {
  ...
  it('changes when aliasIndex content changes', async () => {
    ...
    const indexA: AliasIndex = {
      ...
      byEntity: {
        Piers: { name: 'Piers', type: 'character', aliases: ['broady'], affiliations: [], section: 'Piers', file: 'Characters.md' },
      },
      aliases: { brody: 'Piers', broady: 'Piers' },
      byType: { character: ['Piers'], country: [], deity: [], faction: [], patron: [], location: [], other: [] },
    ...
    const indexB: AliasIndex = {
      ...
      byEntity: {
        ...indexA.byEntity,
        Pernille: { name: 'Pernille', type: 'character', aliases: ['kazle'], affiliations: [], section: 'Pernille', file: 'Characters.md' },
      },
      aliases: { ...indexA.aliases, kaziel: 'Pernille', kazle: 'Pernille' },
      byType: { ...indexA.byType, character: ['Piers', 'Pernille'] },
    }
  ...
  it('ignores aliasIndex.builtAt (rebuild timestamp must not invalidate snapshot)', async () => {
    ...
    const sharedEntities = {
      Piers: { name: 'Piers', type: 'character' as const, aliases: ['broady'], affiliations: [], section: 'Piers', file: 'Characters.md' },
    }
    const sharedAliases = { brody: 'Piers', broady: 'Piers' }
    const sharedByType = {
      character: ['Piers'], country: [], deity: [], faction: [], patron: [], location: [], other: [],
    }
    const indexA: AliasIndex = {
      schema: 1,
      builtAt: '2026-05-01T00:00:00Z',
