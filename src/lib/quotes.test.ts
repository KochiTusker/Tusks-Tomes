...
const MULE_EXCHANGE = {
  kind: 'funny',
  exchange: [
    { speaker: 'Wiktoria', line: 'Did you feed the mule this morning?' },
    { speaker: 'Chidi', line: 'I always feed the mule.' },
    { speaker: 'Wiktoria', line: 'Then why is it eating my cloak?' },
    { speaker: 'Chidi', line: "It's a very hungry mule. That is not on me." },
    { speaker: 'Giulia', line: "You can't even see the mule from here." },
    { speaker: 'Chidi', line: 'Have you heard of a window, Giulia?' },
  ],
}
...
describe('normalizeQuote — single line', () => {
  it('keeps the flat shape and trims', () => {
    expect(normalizeQuote({ speaker: '  Giulia ', line: " that's a rock ", kind: 'stupid' })).toEqual({
      speaker: 'Giulia',
      line: "that's a rock",
      kind: 'stupid',
    })
  ...
  it('drops entries missing a speaker or a line', () => {
    expect(normalizeQuote({ speaker: 'Giulia' })).toBeNull()
  ...
  it('carries an optional context sentence', () => {
    expect(normalizeQuote({ speaker: 'Chidi', line: 'Much obliged.', context: ' He had just been healed. ' }))
      .toEqual({ speaker: 'Chidi', line: 'Much obliged.', kind: 'funny', context: 'He had just been healed.' })
  })

  it('omits context entirely when blank', () => {
    expect(normalizeQuote({ speaker: 'Giulia', line: 'no', context: '   ' })).not.toHaveProperty('context')
  })
})
...
describe('normalizeQuote — exchange', () => {
  it('keeps turns in order and derives the participant list + flat fallback', () => {
    ...
    expect(q.exchange!.map((t) => t.speaker)).toEqual([
      'Wiktoria', 'Chidi', 'Wiktoria', 'Chidi', 'Giulia', 'Chidi',
    ])
    // Participants are unique, in order of first appearance.
    expect(q.speaker).toBe('Wiktoria, Chidi & Giulia')
    // The flat `line` still reads as the whole exchange for any consumer
    // that only understands the pre-exchange shape.
    expect(q.line).toContain('Wiktoria: "Did you feed the mule this morning?"')
    expect(q.line).toContain('Chidi: "Have you heard of a window, Giulia?"')
  ...
  it('drops malformed turns but keeps the rest', () => {
    const q = normalizeQuote({
      kind: 'dark',
      exchange: [
        { speaker: 'Wiktoria', line: 'We march.' },
        { speaker: 'Chidi' },
        'garbage',
        { line: 'no speaker' },
        { speaker: 'Giulia', line: 'Fine.' },
      ],
    })!
  ...
  it('folds a one-turn exchange back to the flat shape', () => {
    const q = normalizeQuote({ kind: 'stupid', exchange: [{ speaker: 'Chidi', line: 'Solo.' }] })
    expect(q).toEqual({ speaker: 'Chidi', line: 'Solo.', kind: 'stupid' })
...
describe('appendNovelQuotes', () => {
  const single: Quote = { speaker: 'Giulia', line: "that's a rock", kind: 'stupid' }

  it('appends novel entries', () => {
    const out = appendNovelQuotes([single], [{ speaker: 'Chidi', line: 'The mule agrees.', kind: 'funny' }])
  ...
  it('drops a standalone line that already sits inside an accumulated exchange', () => {
    ...
    const dupOfATurn: Quote = { speaker: 'Giulia', line: "You can't even see the mule from here.", kind: 'funny' }
  ...
  it('drops an exchange that repeats a line already captured standalone', () => {
    const alreadyHave: Quote = { speaker: 'Chidi', line: 'I always feed the mule.', kind: 'funny' }
    const exchange = normalizeQuote(MULE_EXCHANGE)!
...
describe('quoteKeys', () => {
  it('claims one key per turn plus the entry key', () => {
    ...
    expect(quoteKeys({ speaker: 'Giulia', line: 'x' })).toEqual(['Giulia::x'])
  })
})

describe('quoteToPlainText', () => {
  it('quotes a single line', () => {
    expect(quoteToPlainText({ speaker: 'Giulia', line: 'a rock' })).toBe('Giulia: "a rock"')
  })

  it('renders an exchange without nesting quotes inside quotes', () => {
    const text = quoteToPlainText(normalizeQuote(MULE_EXCHANGE)!)
    expect(text.startsWith('Wiktoria: "Did you feed the mule')).toBe(true)
    expect(text).not.toContain('""')
  })

  it('prefixes context when present', () => {
    expect(quoteToPlainText({ speaker: 'Chidi', line: 'Thanks.', context: 'He was just healed.' }))
      .toBe('(He was just healed.) Chidi: "Thanks."')
  })
})
