...
const MULE_EXCHANGE = {
  kind: 'funny',
  exchange: [
    { speaker: 'Pernille', line: 'Did you feed the mule this morning?' },
    { speaker: 'Anouk', line: 'I always feed the mule.' },
    { speaker: 'Pernille', line: 'Then why is it eating my cloak?' },
    { speaker: 'Anouk', line: "It's a very hungry mule. That is not on me." },
    { speaker: 'Kiona', line: "You can't even see the mule from here." },
    { speaker: 'Anouk', line: 'Have you heard of a window, Kiona?' },
  ],
}
...
describe('normalizeQuote — single line', () => {
  it('keeps the flat shape and trims', () => {
    expect(normalizeQuote({ speaker: '  Kiona ', line: " that's a rock ", kind: 'stupid' })).toEqual({
      speaker: 'Kiona',
      line: "that's a rock",
      kind: 'stupid',
    })
  ...
  it('drops entries missing a speaker or a line', () => {
    expect(normalizeQuote({ speaker: 'Kiona' })).toBeNull()
  ...
  it('carries an optional context sentence', () => {
    expect(normalizeQuote({ speaker: 'Anouk', line: 'Much obliged.', context: ' He had just been healed. ' }))
      .toEqual({ speaker: 'Anouk', line: 'Much obliged.', kind: 'funny', context: 'He had just been healed.' })
  })

  it('omits context entirely when blank', () => {
    expect(normalizeQuote({ speaker: 'Kiona', line: 'no', context: '   ' })).not.toHaveProperty('context')
  })
})
...
describe('normalizeQuote — exchange', () => {
  it('keeps turns in order and derives the participant list + flat fallback', () => {
    ...
    expect(q.exchange!.map((t) => t.speaker)).toEqual([
      'Pernille', 'Anouk', 'Pernille', 'Anouk', 'Kiona', 'Anouk',
    ])
    // Participants are unique, in order of first appearance.
    expect(q.speaker).toBe('Pernille, Anouk & Kiona')
    // The flat `line` still reads as the whole exchange for any consumer
    // that only understands the pre-exchange shape.
    expect(q.line).toContain('Pernille: "Did you feed the mule this morning?"')
    expect(q.line).toContain('Anouk: "Have you heard of a window, Kiona?"')
  ...
  it('drops malformed turns but keeps the rest', () => {
    const q = normalizeQuote({
      kind: 'dark',
      exchange: [
        { speaker: 'Pernille', line: 'We march.' },
        { speaker: 'Anouk' },
        'garbage',
        { line: 'no speaker' },
        { speaker: 'Kiona', line: 'Fine.' },
      ],
    })!
  ...
  it('folds a one-turn exchange back to the flat shape', () => {
    const q = normalizeQuote({ kind: 'stupid', exchange: [{ speaker: 'Anouk', line: 'Solo.' }] })
    expect(q).toEqual({ speaker: 'Anouk', line: 'Solo.', kind: 'stupid' })
...
describe('appendNovelQuotes', () => {
  const single: Quote = { speaker: 'Kiona', line: "that's a rock", kind: 'stupid' }

  it('appends novel entries', () => {
    const out = appendNovelQuotes([single], [{ speaker: 'Anouk', line: 'The mule agrees.', kind: 'funny' }])
  ...
  it('drops a standalone line that already sits inside an accumulated exchange', () => {
    ...
    const dupOfATurn: Quote = { speaker: 'Kiona', line: "You can't even see the mule from here.", kind: 'funny' }
  ...
  it('drops an exchange that repeats a line already captured standalone', () => {
    const alreadyHave: Quote = { speaker: 'Anouk', line: 'I always feed the mule.', kind: 'funny' }
    const exchange = normalizeQuote(MULE_EXCHANGE)!
...
describe('quoteKeys', () => {
  it('claims one key per turn plus the entry key', () => {
    ...
    expect(quoteKeys({ speaker: 'Kiona', line: 'x' })).toEqual(['Kiona::x'])
  })
})

describe('quoteToPlainText', () => {
  it('quotes a single line', () => {
    expect(quoteToPlainText({ speaker: 'Kiona', line: 'a rock' })).toBe('Kiona: "a rock"')
  })

  it('renders an exchange without nesting quotes inside quotes', () => {
    const text = quoteToPlainText(normalizeQuote(MULE_EXCHANGE)!)
    expect(text.startsWith('Pernille: "Did you feed the mule')).toBe(true)
    expect(text).not.toContain('""')
  })

  it('prefixes context when present', () => {
    expect(quoteToPlainText({ speaker: 'Anouk', line: 'Thanks.', context: 'He was just healed.' }))
      .toBe('(He was just healed.) Anouk: "Thanks."')
  })
})
