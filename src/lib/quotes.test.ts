...
const MULE_EXCHANGE = {
  ...
  exchange: [
    ...
    { speaker: 'Zainab', line: 'Then why is it eating my cloak?' },
    { speaker: 'Cassian', line: "It's a very hungry mule. That is not on me." },
    { speaker: 'Hiroko', line: "You can't even see the mule from here." },
    { speaker: 'Cassian', line: 'Have you heard of a window, Hiroko?' },
  ],
}
...
describe('normalizeQuote — single line', () => {
  it('keeps the flat shape and trims', () => {
    expect(normalizeQuote({ speaker: '  Hiroko ', line: " that's a rock ", kind: 'stupid' })).toEqual({
      speaker: 'Hiroko',
      line: "that's a rock",
      kind: 'stupid',
    })
  ...
  it('drops entries missing a speaker or a line', () => {
    expect(normalizeQuote({ speaker: 'Hiroko' })).toBeNull()
  ...
  it('carries an optional context sentence', () => {
    expect(normalizeQuote({ speaker: 'Cassian', line: 'Much obliged.', context: ' He had just been healed. ' }))
      .toEqual({ speaker: 'Cassian', line: 'Much obliged.', kind: 'funny', context: 'He had just been healed.' })
  })

  it('omits context entirely when blank', () => {
    expect(normalizeQuote({ speaker: 'Hiroko', line: 'no', context: '   ' })).not.toHaveProperty('context')
  })
})
...
describe('normalizeQuote — exchange', () => {
  it('keeps turns in order and derives the participant list + flat fallback', () => {
    ...
    expect(q.exchange!.map((t) => t.speaker)).toEqual([
      'Zainab', 'Cassian', 'Zainab', 'Cassian', 'Hiroko', 'Cassian',
    ])
    // Participants are unique, in order of first appearance.
    expect(q.speaker).toBe('Zainab, Cassian & Hiroko')
    // The flat `line` still reads as the whole exchange for any consumer
    // that only understands the pre-exchange shape.
    expect(q.line).toContain('Zainab: "Did you feed the mule this morning?"')
    expect(q.line).toContain('Cassian: "Have you heard of a window, Hiroko?"')
  ...
  it('drops malformed turns but keeps the rest', () => {
    const q = normalizeQuote({
      kind: 'dark',
      exchange: [
        { speaker: 'Zainab', line: 'We march.' },
        { speaker: 'Cassian' },
        'garbage',
        { line: 'no speaker' },
        { speaker: 'Hiroko', line: 'Fine.' },
      ],
    })!
  ...
  it('folds a one-turn exchange back to the flat shape', () => {
    const q = normalizeQuote({ kind: 'stupid', exchange: [{ speaker: 'Cassian', line: 'Solo.' }] })
    expect(q).toEqual({ speaker: 'Cassian', line: 'Solo.', kind: 'stupid' })
...
describe('appendNovelQuotes', () => {
  const single: Quote = { speaker: 'Hiroko', line: "that's a rock", kind: 'stupid' }

  it('appends novel entries', () => {
    const out = appendNovelQuotes([single], [{ speaker: 'Cassian', line: 'The mule agrees.', kind: 'funny' }])
  ...
  it('drops a standalone line that already sits inside an accumulated exchange', () => {
    ...
    const dupOfATurn: Quote = { speaker: 'Hiroko', line: "You can't even see the mule from here.", kind: 'funny' }
  ...
  it('drops an exchange that repeats a line already captured standalone', () => {
    const alreadyHave: Quote = { speaker: 'Cassian', line: 'I always feed the mule.', kind: 'funny' }
    const exchange = normalizeQuote(MULE_EXCHANGE)!
...
describe('quoteKeys', () => {
  it('claims one key per turn plus the entry key', () => {
    ...
    expect(quoteKeys({ speaker: 'Hiroko', line: 'x' })).toEqual(['Hiroko::x'])
  })
})

describe('quoteToPlainText', () => {
  it('quotes a single line', () => {
    expect(quoteToPlainText({ speaker: 'Hiroko', line: 'a rock' })).toBe('Hiroko: "a rock"')
  })

  it('renders an exchange without nesting quotes inside quotes', () => {
    const text = quoteToPlainText(normalizeQuote(MULE_EXCHANGE)!)
    expect(text.startsWith('Zainab: "Did you feed the mule')).toBe(true)
    expect(text).not.toContain('""')
  })

  it('prefixes context when present', () => {
    expect(quoteToPlainText({ speaker: 'Cassian', line: 'Thanks.', context: 'He was just healed.' }))
      .toBe('(He was just healed.) Cassian: "Thanks."')
  })
})
