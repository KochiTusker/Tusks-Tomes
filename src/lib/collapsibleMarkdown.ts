...
// a <details open><summary><h2>...</h2></summary><div>...</div></details> block.
//
// Sections open by DEFAULT. They used to render closed, which turned every
// page into a stack of shut drawers: reading a doc end to end meant clicking
// each heading in turn, and nothing on screen told you which drawer held what
// you wanted. Open-by-default reads as ordinary prose, and the chevron is
// still there for anyone who wants to fold a section away.
...
export function wrapH2SectionsInDetails(md: string): string {
  // Hands-off on structure: the author already wrote <details> blocks. Still
  // force them open, so a hand-authored page and an auto-wrapped one behave
  // the same — a reader should never have to learn which is which.
  if (/<details[\s>]/i.test(md)) {
    return md.replace(/<details(?![^>]*\bopen\b)([^>]*)>/gi, '<details$1 open>')
  }

  // Split on either ending: a CRLF working tree leaves a trailing \r that
  // the fence tracking would not match, silently mis-detecting code blocks.
  ...
  for (const line of lines) {
    ...
    if (h2) {
      ...
      out.push('<details class="docs-section" open>')
