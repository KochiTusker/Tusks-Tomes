...
describe('annotateChunk — phonetic alias matching', () => {
  ...
  it('does NOT match different-first-sound names ("Yannick" should NOT match "Bhargo")', () => {
    // Both M-codes start with B+R = PR in metaphone. The first-sound
    // discipline only rules out when the FIRST sound differs; Yannick/Bhargo
    ...
    const result = annotateChunk('[Yannick] hit the goblin.', idx)
    expect(result.candidates).toHaveLength(0)
  })
