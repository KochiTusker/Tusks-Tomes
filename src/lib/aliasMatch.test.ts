...
describe('annotateChunk — phonetic alias matching', () => {
  ...
  it('does NOT match different-first-sound names ("Piers" should NOT match "Bhargo")', () => {
    // Both M-codes start with B+R = PR in metaphone. The first-sound
    // discipline only rules out when the FIRST sound differs; Piers/Bhargo
    ...
    const result = annotateChunk('[Piers] hit the goblin.', idx)
    expect(result.candidates).toHaveLength(0)
  })
