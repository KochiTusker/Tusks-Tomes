...
export function cloudProfileFor(
  ...
  // Claude Code runs Claude models via the user's subscription — reuse the
  // Claude chunk-size profile (same model family, same context behaviour).
  if (provider === 'claudeCode') return 'claude'
  // Codex runs GPT-family models via the user's ChatGPT subscription —
  // reuse the OpenAI sizing profile for the same reason.
  return 'openai'
}
