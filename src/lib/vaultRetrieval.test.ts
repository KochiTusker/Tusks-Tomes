...
const DOCS: KBDocument[] = [
  ...
  doc('d2', 'people/Yuzuki Osk.md', 'A paladin. Serves Aveline reluctantly.'),
  ...
  doc('d5', 'logs/Session 27.md', 'Aveline appeared. Yuzuki Osk knelt.'),
  doc('d6', 'logs/Session 28.md', 'Yuzuki Osk fled the Crimson Cathedral.'),
...
const INDEX: AliasIndex = {
  ...
  byEntity: {
    ...
    'Yuzuki Osk': {
      name: 'Yuzuki Osk', type: 'character', aliases: ['Yuzuki'],
      affiliations: [], section: 'Yuzuki', file: 'people/Yuzuki Osk.md',
    },
  },
  aliases: {
    daraksha: 'Aveline', 'the pale ledger': 'Aveline',
    'lady of the gleaming abyss': 'Aveline', brugo: 'Yuzuki Osk',
  },
  byType: { character: ['Yuzuki Osk'], country: [], deity: ['Aveline'], faction: [], patron: [], location: [], other: [] },
...
describe('retrieveForText — Tier 0 (no profile, any vault layout)', () => {
  it('retrieves notes whose title appears in the text', () => {
    const r = retrieveForText('Yuzuki Osk swung his hammer.', DOCS, { index: INDEX })
  ...
  it('carries the previous chunk forward so pronoun-only follow-ups stay grounded', () => {
    const first = retrieveForText('Yuzuki Osk arrived.', DOCS, { index: INDEX })
  ...
  it('reduces payload substantially versus sending everything', () => {
    const r = retrieveForText('Yuzuki Osk knelt.', DOCS, { index: INDEX })
...
describe('retrieveForText — profile is advisory, never lossy', () => {
  it('demotes reference material when a profile marks it', () => {
    const withProfile = retrieveForText('Yuzuki Osk knelt.', DOCS, { index: INDEX, profile: PROFILE })
  ...
  it('SAFETY: a literal match survives an exhausted budget', () => {
    const r = retrieveForText('Yuzuki Osk and Aveline and the Crimson Cathedral.', DOCS, {
  ...
  it('drops only non-literal notes to satisfy the budget', () => {
    const r = retrieveForText('Yuzuki Osk knelt.', DOCS, {
...
describe('computeCoreNotes', () => {
  it('ranks the most-referenced notes without knowing any folder names', () => {
    // Aveline is mentioned by Yuzuki, the Cathedral, both logs and the retired note.
