...
const DOCS: KBDocument[] = [
  ...
  doc('d2', 'people/Cassian Osk.md', 'A paladin. Serves Aveline reluctantly.'),
  ...
  doc('d5', 'logs/Session 27.md', 'Aveline appeared. Cassian Osk knelt.'),
  doc('d6', 'logs/Session 28.md', 'Cassian Osk fled the Crimson Cathedral.'),
...
const INDEX: AliasIndex = {
  ...
  byEntity: {
    ...
    'Cassian Osk': {
      name: 'Cassian Osk', type: 'character', aliases: ['Cassian'],
      affiliations: [], section: 'Cassian', file: 'people/Cassian Osk.md',
    },
  },
  aliases: {
    daraksha: 'Aveline', 'the pale ledger': 'Aveline',
    'lady of the gleaming abyss': 'Aveline', brugo: 'Cassian Osk',
  },
  byType: { character: ['Cassian Osk'], country: [], deity: ['Aveline'], faction: [], patron: [], location: [], other: [] },
...
describe('retrieveForText — Tier 0 (no profile, any vault layout)', () => {
  it('retrieves notes whose title appears in the text', () => {
    const r = retrieveForText('Cassian Osk swung his hammer.', DOCS, { index: INDEX })
  ...
  it('carries the previous chunk forward so pronoun-only follow-ups stay grounded', () => {
    const first = retrieveForText('Cassian Osk arrived.', DOCS, { index: INDEX })
  ...
  it('reduces payload substantially versus sending everything', () => {
    const r = retrieveForText('Cassian Osk knelt.', DOCS, { index: INDEX })
...
describe('retrieveForText — profile is advisory, never lossy', () => {
  it('demotes reference material when a profile marks it', () => {
    const withProfile = retrieveForText('Cassian Osk knelt.', DOCS, { index: INDEX, profile: PROFILE })
  ...
  it('SAFETY: a literal match survives an exhausted budget', () => {
    const r = retrieveForText('Cassian Osk and Aveline and the Crimson Cathedral.', DOCS, {
  ...
  it('drops only non-literal notes to satisfy the budget', () => {
    const r = retrieveForText('Cassian Osk knelt.', DOCS, {
...
describe('computeCoreNotes', () => {
  it('ranks the most-referenced notes without knowing any folder names', () => {
    // Aveline is mentioned by Cassian, the Cathedral, both logs and the retired note.
