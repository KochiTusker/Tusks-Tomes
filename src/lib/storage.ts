...
export function safeSet(key: string, value: unknown): boolean {
  ...
  return guardedWrite(key, serialized)
}

/** Quota-guarded write for values stored as RAW strings (not JSON) — the
 *  verbose-log flags ('1') and the failsafe model id predate the JSON
 *  convention, and rewriting their format would orphan every existing
 *  value. Same warning + failure events as safeSet. */
export function safeSetRaw(key: string, rawValue: string): boolean {
  return guardedWrite(key, rawValue)
}

function guardedWrite(key: string, serialized: string): boolean {
