 ...
 *  is access control at the remote, not what's typed here. The server
 *  flips both the in-memory session grant AND the persisted preference
 *  to "dev". The grant resets on server restart, so the operator
 ...
 *  After this, the email must be re-entered to use dev. */
