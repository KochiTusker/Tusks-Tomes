...
describe('cloudProfileFor', () => {
  ...
  it('maps the subscription CLIs onto their model family rows', () => {
    // The row names describe the MODEL FAMILY, not the billing route, so they
    // outlive the direct keys that used to share them.
    expect(cloudProfileFor('claudeCode', undefined)).toBe('claude')
    expect(cloudProfileFor('codex', undefined)).toBe('openai')
  })
})
...
describe('chunkSizeFor', () => {
  ...
  it('returns the right per-provider size for cloud routing', () => {
    ...
    expect(chunkSizeFor({ phase: 'p3', isLocal: false, cloudProvider: 'claudeCode' }))
      .toBe(cloudChunkSize('claude', 'p3'))
    expect(chunkSizeFor({ phase: 'p6', isLocal: false, cloudProvider: 'codex' }))
...
describe('cloudSizingKeyFor', () => {
  it('composes profile and tier into a single key', () => {
    ...
    expect(cloudSizingKeyFor('claudeCode', undefined, 'frontier')).toBe('claude:frontier')
    expect(cloudSizingKeyFor('codex', undefined, 'fast')).toBe('openai:fast')
  })
})
...
describe('chunk sizes — model tier dimension', () => {
  it('omitting modelTier returns the flagship-row size (back-compat lock)', () => {
    ...
    for (const phase of ['p1', 'p2', 'p3', 'p4', 'p6'] as const) {
      ...
      expect(chunkSizeFor({ phase, isLocal: false, cloudProvider: 'claudeCode' }))
        .toBe(cloudChunkSize('claude', phase, 'flagship'))
      expect(chunkSizeFor({ phase, isLocal: false, cloudProvider: 'codex' }))
