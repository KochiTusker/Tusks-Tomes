...
const GROUNDED_PROSE_PER_CHUNK = (i: number) =>
  `[Anwen (Tadeo)] In Waterdeep, Anwen rolls a perception check while Yannick watches. ` +
...
const PHASE2_QUESTIONS_PER_CHUNK = (i: number) =>
  JSON.stringify([
    {
      id: `q-${i}-1`,
      question: `What did Anwen actually do at the Crimson Cathedral in chunk ${i}?`,
...
const CHRONICLE_PROSE_PER_CHUNK = (i: number) =>
  `The chronicle of chunk ${i} unfolds: Anwen stepped through the gates of the Crimson ` +
  `Cathedral, the Sun Blade humming faintly at his hip. Yannick followed, axe gleaming. ` +
...
const PHASE4_EXTRAS_PER_CHUNK = (i: number) =>
  JSON.stringify({
    jests: [`Chunk ${i}: Anwen forgot to bring rope, again.`],
...
const PHASE6_CONDENSE_PER_CHUNK = (i: number) =>
  JSON.stringify({
    narrative:
      ...
      `Anwen leading with the Sun Blade. Yannick followed, axe drawn. They paused at the ` +
    ...
    bulletPoints: [
      ...
      `Chunk ${i} — bullet B: Anwen found the inscription`,
    ],
  })
...
describe('pipeline integration — all 6 phases on the 24KB fixture', () => {
  ...
  it('fixture is the expected ~24KB shape (sanity-check)', () => {
    ...
    expect(TRANSCRIPT_24KB).toMatch(/^\[Anwen \(Player\)\]/)
  })
  ...
  describe('Phase 6 — condense', () => {
    it('returns CondenseOutput { narrative, bulletPoints } when chronicle is supplied', async () => {
      ...
      const chronicle = (
        'Anwen walked into the Crimson Cathedral. Yannick followed, axe in hand. ' +
