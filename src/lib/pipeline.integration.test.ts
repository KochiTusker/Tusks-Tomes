...
const GROUNDED_PROSE_PER_CHUNK = (i: number) =>
  `[Pernille (Bronwen)] In Waterdeep, Pernille rolls a perception check while Piers watches. ` +
...
const PHASE2_QUESTIONS_PER_CHUNK = (i: number) =>
  JSON.stringify([
    {
      id: `q-${i}-1`,
      question: `What did Pernille actually do at the Crimson Cathedral in chunk ${i}?`,
...
const CHRONICLE_PROSE_PER_CHUNK = (i: number) =>
  `The chronicle of chunk ${i} unfolds: Pernille stepped through the gates of the Crimson ` +
  `Cathedral, the Sun Blade humming faintly at his hip. Piers followed, axe gleaming. ` +
...
const PHASE4_EXTRAS_PER_CHUNK = (i: number) =>
  JSON.stringify({
    jests: [`Chunk ${i}: Pernille forgot to bring rope, again.`],
...
const PHASE6_CONDENSE_PER_CHUNK = (i: number) =>
  JSON.stringify({
    narrative:
      ...
      `Pernille leading with the Sun Blade. Piers followed, axe drawn. They paused at the ` +
    ...
    bulletPoints: [
      ...
      `Chunk ${i} — bullet B: Pernille found the inscription`,
    ],
  })
...
describe('pipeline integration — all 6 phases on the 24KB fixture', () => {
  ...
  it('fixture is the expected ~24KB shape (sanity-check)', () => {
    ...
    expect(TRANSCRIPT_24KB).toMatch(/^\[Pernille \(Player\)\]/)
  })
  ...
  describe('Phase 6 — condense', () => {
    it('returns CondenseOutput { narrative, bulletPoints } when chronicle is supplied', async () => {
      ...
      const chronicle = (
        'Pernille walked into the Crimson Cathedral. Piers followed, axe in hand. ' +
