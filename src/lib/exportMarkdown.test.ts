...
describe('buildMarkdown (full export — back-compat)', () => {
  ...
  it('nests an exchange under its participants instead of one run-on line', () => {
    ...
    expect(md).toContain('- **Pernille, Anouk & Kiona** — _The mule had gone missing two days earlier._')
    expect(md).toContain('  - **Pernille:** "Did you feed the mule this morning?"')
    expect(md).toContain('  - **Anouk:** "I always feed the mule."')
    expect(md).toContain('  - **Kiona:** "You can\'t even see the mule from here."')
