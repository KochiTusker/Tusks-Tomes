...
export const CLAUDE_CODE_ADDON = 'claude-code-addon'
export const CODEX_ADDON = 'codex-addon'
export const OBSIDIAN_ADDON = 'obsidian-vault-addon'

/** Which add-on backs a subscription target. */
export function addonForSubscription(sub: SubscriptionTarget): string {
  return sub === 'claudeCode' ? CLAUDE_CODE_ADDON : CODEX_ADDON
}
...
export function buildSetupPlan(current: CurrentSetup, answers: SetupAnswers): SetupPlan {
  ...
  if (sub !== 'none') {
    ...
    const addon = addonForSubscription(sub)
    const ready = current.addonsReady[addon] === true
    ...
    steps.push({
      id: `addon-${addon}`,
      kind: 'addon',
      label: `Enable the ${SUB_LABEL[sub]} add-on`,
      detail: 'Links the app to the CLI you are already signed in to. No API key is used for these phases.',
      before: ready ? 'already enabled' : 'not enabled',
      after: ready ? 'unchanged' : 'enabled',
      skipped: ready,
    })
  } else {
  ...
  if (answers.vaultPath) {
    const ready = current.addonsReady[OBSIDIAN_ADDON] === true
    steps.push({
      id: `addon-${OBSIDIAN_ADDON}`,
      kind: 'addon',
      label: 'Enable the Obsidian Vault lore source',
      detail: 'Lets the chronicler ground names and places against your own notes. The vault is read-only.',
      before: ready ? 'already enabled' : 'not enabled',
      after: ready ? 'unchanged' : 'enabled',
      skipped: ready,
    })
