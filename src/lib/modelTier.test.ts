...
describe('classifyModelTier — claude', () => {
  it('classifies opus as frontier', () => {
    expect(classifyModelTier('claude-opus-4-7', 'claudeCode')).toBe('frontier')
    expect(classifyModelTier('claude-3-opus-20240229', 'claudeCode')).toBe('frontier')
  })
  it('classifies sonnet as flagship', () => {
    expect(classifyModelTier('claude-sonnet-4-6', 'claudeCode')).toBe('flagship')
    expect(classifyModelTier('claude-3-5-sonnet-20241022', 'claudeCode')).toBe('flagship')
  })
  it('classifies haiku as fast', () => {
    expect(classifyModelTier('claude-haiku-4-5-20251001', 'claudeCode')).toBe('fast')
    expect(classifyModelTier('claude-3-5-haiku-20241022', 'claudeCode')).toBe('fast')
  })
  it('defaults unknown claude strings to flagship', () => {
    expect(classifyModelTier('claude-future', 'claudeCode')).toBe('flagship')
  })
})

describe('classifyModelTier — openai', () => {
  it('classifies mini/nano as fast', () => {
    expect(classifyModelTier('gpt-5-mini', 'codex')).toBe('fast')
    expect(classifyModelTier('gpt-5-nano', 'codex')).toBe('fast')
    expect(classifyModelTier('gpt-4o-mini', 'codex')).toBe('fast')
  })
  it('classifies gpt-5 and gpt-4o as flagship', () => {
    expect(classifyModelTier('gpt-5', 'codex')).toBe('flagship')
    expect(classifyModelTier('gpt-4o', 'codex')).toBe('flagship')
    expect(classifyModelTier('gpt-4-turbo', 'codex')).toBe('flagship')
  })
  it('defaults unknown openai strings to flagship', () => {
    expect(classifyModelTier('gpt-future', 'codex')).toBe('flagship')
  })
})
...
describe('classifyModelTier — edge cases', () => {
  it('handles empty string as flagship', () => {
    ...
    expect(classifyModelTier('', 'claudeCode')).toBe('flagship')
    expect(classifyModelTier('', 'codex')).toBe('flagship')
  })
  it('is case-insensitive', () => {
    expect(classifyModelTier('GEMINI-2.5-FLASH', 'gemini')).toBe('fast')
    expect(classifyModelTier('Claude-Haiku-4-5', 'claudeCode')).toBe('fast')
    expect(classifyModelTier('GPT-5-MINI', 'codex')).toBe('fast')
  })
})
