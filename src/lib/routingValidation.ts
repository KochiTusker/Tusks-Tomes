...
function isKnownPricedModel(provider: string | undefined, modelId: string | undefined): boolean {
  ...
  // Everything else — the subscription CLIs and OpenRouter's ~400-model
  // catalogue — has no fixed rate table to check against, so model names are
  // left to the downstream pipeline rather than guessed at here.
  return true
}
