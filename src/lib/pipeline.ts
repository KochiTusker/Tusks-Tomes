  ...
  try {
    for (let i = startIdx; i < total; i++) {
      ...
      if (args.delayMs !== undefined) {
        ...
        delay = provider.getNextDelayMs?.(estimatedInputTokens, mult) ?? 0
      }
      if (delay > 0) {
        ...
        if (args.cloudProvider === 'claudeCode') {
          ...
          if (check.refused) {
            ...
            if (getClaudeFailsafeEnabled()) {
              const gem = getCloudProvider('gemini', { geminiTier: args.geminiTier ?? 'auto' })
              const gemHasKey = (gem as { hasKey?: () => boolean }).hasKey?.() ?? false
              if (gemHasKey) {
                // Flash, deliberately — the failsafe only ever redoes the ONE
                // refused chunk (small in, small out), so Flash (free or paid)
                // keeps it cheap. The chunk's own transcript span + phase prompt
                // give Gemini exactly where the refusal was and what was said.
                const gemModel = 'gemini-2.5-flash'
                try {
                  ...
                  const gemRes = await gem.generate(
                    ...
                    {
                      signal: args.signal,
                      contextLabel: args.contextLabel(i, total) + ' [gemini restore — claude refusal]',
                      onProviderEvent: args.onProviderEvent,
                      onRetry: args.onRetry,
                    },
                  )
...
export async function runPhase6(args: {
  ...
  const retrievedKb =
    !local && args.retrieveVaultKb
      ? retrieveForText(chronicle, kb, { index: args.aliasIndex ?? null }).docs
      : kb
