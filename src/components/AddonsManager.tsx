...
export function AddonsManager() {
      ...
      while (true) {
        ...
        for (const block of parts) {
          ...
          if (event === 'line') {
            ...
            if (exitCode === 0) {
              toast.success(`${addon.displayName} installed. Restart npm run dev to activate.`)
  ...
  async function toggle(addon: AddonEntry, next: boolean) {
    ...
    try {
      ...
      toast.success(
        `${addon.displayName} ${next ? 'enabled' : 'disabled'}. Restart npm run dev to ${
  ...
  async function uninstall(addon: AddonEntry) {
    ...
    try {
      ...
      toast.success(`${addon.displayName} uninstalled. Restart npm run dev to deactivate.`)
      setPendingRestart(true)
      clearLog(addon.name)
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Optional features you can install to extend Tusk's Tomes. Each add-on is
          independent — install only what you need. Changes take effect after restarting
          the server.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* One-click promise — honest about which add-ons actually download
            anything. Personas and Local LLMs just flip marker files; Audio
            Transcription is the one that pulls a Python venv. */}
        <div className="rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
          <p>
            <strong className="text-foreground">Most add-ons install instantly</strong> — just a
            marker file in your per-user app-data folder, no npm, no admin, no downloads. Uninstall
            removes the marker; the source code stays on disk in case you change your mind.
          </p>
          <p className="mt-1.5">
            <strong className="text-foreground">Audio Transcription is the exception:</strong> it
            downloads a Python virtual environment (~1.5 GB) for the Whisper sidecar. Install
            progress streams live below the row.
          </p>
        </div>

        {pendingRestart && (
          <div className="flex items-center justify-between rounded-md border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-sm">
            <span>Restart <code>npm run dev</code> for changes to take effect.</span>
