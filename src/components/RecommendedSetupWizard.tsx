...
import { CpuWhisperWarning, PythonBlockedNotice } from '@/components/WhisperPrereqs'
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        {!loading && step === 'whisper' && current && (
          <div className="space-y-3 text-sm">
            ...
            {answers.installWhisper && current.python && !current.python.supported && (
              <PythonBlockedNotice found={current.python.found} version={current.python.version} />
            )}
            {!current.gpu.detected && answers.installWhisper && (
              <CpuWhisperWarning
                accepted={answers.acceptedCpuWhisper}
                onAcceptedChange={(next) =>
                  setAnswers((a) => ({ ...a, acceptedCpuWhisper: next }))
                }
              />
            )}
          </div>
