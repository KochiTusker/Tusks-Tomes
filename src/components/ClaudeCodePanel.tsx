...
import {
  FAILSAFE_TARGETS,
  getClaudeFailsafeEnabled,
  getFailsafeTarget,
  setClaudeFailsafeEnabled,
  setFailsafeTarget,
} from '@/lib/claudeFailsafe'
...
export function ClaudeCodePanel() {
  ...
  const [repairModel, setRepairModel] = useState(() => getFailsafeTarget().modelId)
  const [geminiOk, setGeminiOk] = useState(false)
  useEffect(() => {
    setFailsafeOn(getClaudeFailsafeEnabled())
    setRepairModel(getFailsafeTarget().modelId)
        ...
        <div className="rounded-md border border-border p-3">
          <label className="flex cursor-pointer items-start gap-2">
            ...
            <span className="text-sm">
              <span className="flex items-center gap-1 font-medium">
                ...
                Explicit-content failsafe
              </span>
              <span className="text-muted-foreground">
                If Claude Code refuses or blanks a chunk, redo just that chunk on a permissive
                model mid-run; and after a run where a refusal was detected, offer a pass that
                reconciles the chronicle against the raw transcript to restore softened or omitted
                gore, profanity and sexual references. Off by default — ideally never needed.
              </span>
            </span>
          </label>
          ...
          {failsafeOn && (
            <div className="mt-3 border-t border-border pt-3">
              <label className="block text-xs font-medium" htmlFor="failsafe-model">
                Repair with
              </label>
              <select
                id="failsafe-model"
                value={repairModel}
                onChange={(e) => {
                  setRepairModel(e.target.value)
                  setFailsafeTarget(e.target.value)
                }}
                className="mt-1 h-9 w-full rounded-md border border-input bg-background px-2 text-sm"
              >
                {FAILSAFE_TARGETS.map((t) => (
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {FAILSAFE_TARGETS.find((t) => t.modelId === repairModel)?.why}
              </p>
              <p className="mt-1.5 text-xs text-muted-foreground">
                Every option here was <strong>measured</strong> writing up graphic violence and
                crude dialogue without softening it. A repair runs because something already
                refused the material, so an unproven model would risk refusing it twice.
              </p>
              {!geminiOk &&
                FAILSAFE_TARGETS.find((t) => t.modelId === repairModel)?.provider === 'gemini' && (
                  <p className="mt-1.5 text-amber-700 dark:text-amber-300">
                    ⚠ No Gemini key configured. Pick an OpenRouter option above, or add a Gemini key
