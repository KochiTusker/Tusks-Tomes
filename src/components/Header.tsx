        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              max={9999}
              onChange={(e) => {
                // Math.max(1, …) already rejected zero and negatives, but
                // 3.7 and 999999999 both survived — and the session number
                // lands in the chronicle text and in export filenames.
                const n = Math.floor(Number(e.target.value))
                if (!Number.isFinite(n)) return
                setSessionNumber(Math.min(9999, Math.max(1, n)))
              }}
              className="w-24"
            />
          </div>
          <div className="flex min-w-0 max-w-full flex-wrap items-end gap-2 self-end">
            <ProviderModelsLink />
          </div>
        </div>
      </div>
