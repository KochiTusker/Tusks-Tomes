  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          // Alerts first: the rewrite emits <div> blocks, and the section
          // wrapper passes any non-H2 line through untouched. Doing it the
          // other way round would hide alerts that sit in the preamble.
          setActiveDoc({
            ...body,
...
/** The shelves, in the order a reader meets them, with the label each one
 *  shows. Kept in step with the same list in HelpLanding and in the site
 *  build's NAV_GROUPS — three surfaces, one running order.
 *
 *  Raw folder paths used to be the labels here, so the sidebar read
 *  "DOCS/GETTING-STARTED" and sorted alphabetically, which put "About" above
 *  "Getting started" and buried the page a new reader wants first. */
const SIDEBAR_SHELVES: Array<{ prefix: string; label: string }> = [
...
function groupDocs(entries: DocEntry[]): Array<{ label: string; entries: DocEntry[] }> {
  ...
  const order = ['Documentation', ...SIDEBAR_SHELVES.map((s) => s.label), 'More']
  return order
    .filter((label) => byGroup.has(label))
    .map((label) => ({ label, entries: byGroup.get(label)! }))
}
