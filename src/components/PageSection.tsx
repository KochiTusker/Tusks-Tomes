 ...
 *  and flex items default to align-items:stretch — which made it exactly
 *  as tall as the whole settings column (measured: 3666px). A stretched
 *  element has no room to move inside its container, so `sticky` had
 *  never once stuck. Shrinking it to content height is what makes the
 *  stickiness real. The max-height + overflow keep a long rail usable on
 *  a short viewport instead of running off the bottom. */
export function PageSectionNav({ sections }: { sections: PageSectionDef[] }) {
  return (
    <nav aria-label="Page sections" className="sticky top-4 hidden max-h-[calc(100vh-2rem)] w-52 shrink-0 self-start overflow-y-auto xl:block">
