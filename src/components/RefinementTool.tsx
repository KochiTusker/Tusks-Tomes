...
import { PerPhaseOverrideBanner } from './PerPhaseOverrideBanner'
...
import { emitActiveProviderChanged } from './ActiveProviderCard'
...
export function RefinementTool() {
        ...
        const message =
          ...
          (dubious
            ? ` ⚠ ${sub.to} is a lite/experimental/latest variant — likely NOT what you picked. ` +
              `Open Hybrid Routing to pin a specific model, OR probe again.`
            : ' Open Hybrid Routing to override.')
   ...
   *
   *  The author's original comment on resumeFromCheckpoint admitted
  ...
  if (isRunningPhaseId(state.status)) {
    ...
    const overrides = sessionRef.current?.perPhaseOverrides ?? []
    return (
      <>
        {overrides.length > 0 && (
          <PerPhaseOverrideBanner
            key={currentRunIdRef.current ?? 'no-run'}
            overrides={overrides}
            runIdSignal={currentRunIdRef.current}
          />
        )}
        {devTruncation && (
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Knowledge Base: {kb.length} document{kb.length === 1 ? '' : 's'}
            {kb.length > 0 && (
              <> · {fmtKbSize(kb.reduce((acc, d) => acc + d.text.length, 0))} of extracted text</>
            )}
            .{' '}
            {kb.length === 0 ? (
              <span className="text-amber-600">
                Drop lore documents into the Tusks-Lore folder (or use the Knowledge Base tab) for best grounding.
              </span>
            ) : (
              <span className="text-xs text-muted-foreground">
                Larger KB = higher LLM cost per chunk. Trim the Tusks-Lore folder or use the optional Condense phase to keep things cheap.
              </span>
            )}
            <br />
            <span className="text-xs">
              Manage campaign-specific mistranscriptions in the Tome of Lore tab → Glossary.
            </span>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <CostEstimatorCard
            ...
            kbChars={kb.reduce((acc, d) => acc + d.text.length, 0)}
          />
