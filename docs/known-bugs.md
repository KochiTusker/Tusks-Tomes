...
1. **✅ Fixed in v1.1.0** — bugs that the v1.1.0 bug-hunt surfaced and resolved, kept here so you can see what changed and why. (Fixes landed in later releases are in the [roadmap's "Recently shipped"](../ROADMAP.md) and the commit log rather than repeated here.)
...
| **Audio Transcription (Whisper)** | Exercised on NVIDIA GPU. CPU-only transcription works but is very slow — see [workflows.md](workflows.md). |
...
[hybrid routing](../ROADMAP.md) exists for, and it keeps the prose quality where
...
**Was:** Every tracked `.sh` script (`setup.sh`, `start.sh`, `scripts/whisper/setup.sh`, `scripts/hooks/pre-push`, `scripts/update/apply.sh`) was 100% CRLF because the maintainer develops on Windows with `core.autocrlf=true`. A Linux / macOS user who cloned from public and ran `bash setup.sh` would hit `\r: command not found` on the first line. **The "clone + run setup" experience was broken on every non-Windows platform.**

**Fixed:** added `.gitattributes` at the repo root that forces `text eol=lf` on `*.sh`, `*.bash`, `*.py`, and `scripts/hooks/*` regardless of the cloning user's git config. Existing CRLF bytes were normalised to LF in-place. Future contributors on any OS will land LF when these file types are touched.
...
- **A 3-hour session costs more than your friend's NotebookLM workflow.** True. We're an open-source app paying per-token at retail rates; NotebookLM is Google subsidising a free tier. We trade higher cost for local-first privacy and per-campaign grounding. See [comparison.md](comparison.md).
- **The chronicle is verbose.** Also by design — see [feedback-chronicle-voice in the FAQ](faq.md#how-a-full-session-goes--what-to-expect-at-each-step). The Phase 6 condense is the version to share; the chronicle is the canonical record.
