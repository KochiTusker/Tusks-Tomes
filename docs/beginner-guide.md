# 🧙 Beginner's guide — setting this up with no terminal experience
...
(~5 minutes) and the add-on install (which downloads about 2 GB, so it depends
...
standalone installer is on the [roadmap](../ROADMAP.md); it isn't built yet,
...
1. **Add an LLM API key.** Settings → API Keys → paste a Gemini / Claude / OpenAI key → **Save**.
   - **Don't have one yet?** See [providers.md](providers.md) for paid-key sign-up walkthroughs. (Gemini Pro moved behind billing in 2026 — a paid key is required for the main pipeline; the project has been engineered to keep that bill small.)
   - **Want to run fully offline?** Install the **Local LLMs** add-on first (Settings → Add-ons → Local LLMs → Install, restart), then install [Ollama](https://ollama.com) and use Settings → Local LLM → **Detect**.
2. *(Optional)* **Enable Audio Transcription.** If you want to upload Craig recordings or transcribe audio files directly: **Settings → Add-ons → Audio Transcription → Install**. This downloads the Whisper sidecar (~2 GB) — requires Python from Step 2. Restart `Start_Tusks_Tomes.bat` when prompted.
3. *(Optional)* **Enable Local LLMs.** If you want to route phases through Ollama, LM Studio, or Unsloth: **Settings → Add-ons → Local LLMs → Install** (no download — the add-on is a feature flag). Then install your runner of choice separately and restart Tomes.
...
6. **Chronicle a session.** Either paste a transcript into the Chronicle tab, or follow one of the [two workflows](workflows.md).
