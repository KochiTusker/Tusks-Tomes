# Add-on isolation — Node and Python sandboxing rules
...
> **The promise to the user:** installing an add-on must never change anything outside this repo's working directory. Uninstalling must remove every byte the install wrote. The host computer's Python, Node, PATH, registry, and home directory stay untouched.
...
| Marker file | `{configDir}/addons.json` | Per-user config dir (`%APPDATA%\tusks-tomes\Config\` on Windows — Roaming; `~/.config/tusks-tomes/` on Linux; `~/Library/Application Support/tusks-tomes/` on macOS), one line per add-on. |
...
<details class="docs-section">
<summary><h2>🛠️ External binaries (Ollama, LM Studio, ffmpeg, …)</h2></summary>
<div class="docs-section-body">
...
- **The add-on writes a marker file** (`{configDir}/<name>.enabled`) so install/uninstall has the same shape as the heavier add-ons. The marker is opt-in metadata, not a precondition.

The existing local-llm-addon (`server/addons/registry.ts:81-114`) is the reference for this pattern.
...
10. Use `child_process.spawn(cmd, args, { shell: true })` with un-quoted args — see [DEP0190 fix](../../src/server/util/updater.ts) for the cmd.exe-quoting helper.
...
- [ ] Fresh clone on a clean Windows VM that has Node 20 + Python 3.11 + nothing else. App starts and the new add-on shows up as "not installed."
...
- [ ] Clicking *Uninstall* removes the entire `vendor/addons/<name>/` tree (or `vendor/python-venv/` for Python add-ons) and the marker in `addons.json`.
- [ ] After uninstall, *re-installing* works (idempotent).
- [ ] The add-on's `isReady()` returns the correct value on every state (not-installed, installed-but-pending-restart, fully-active).
...
- [local-llm add-on](local-llm.md) — the reference external-binary implementation
