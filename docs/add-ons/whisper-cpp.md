...
**So this add-on is a bridge, not an installer.** You build or obtain
whisper.cpp yourself; Tusk's Tomes detects it, checks it, and uses it. Same
arrangement as the [Claude Code](claude-code.md) and [Codex](codex.md) add-ons:
you own the tool, I own the integration.

> **It downloads and installs nothing.** Enabling the add-on writes a single
> marker file. Uninstalling removes that file and the paths you configured —
> your whisper.cpp build and model files are yours and are never touched.
...
> ⚠️ **Downloading an official release will not give you GPU acceleration.**
> The release binaries are CPU-only (or CUDA, if you have NVIDIA — in which
> case use the normal Audio Transcription add-on instead, it's easier). For
> AMD or Intel you have to compile with Vulkan. The add-on reads your binary's
> own capability line and will tell you plainly if it's a CPU-only build, so
> you won't be left wondering why it's slow.
...
1. **Settings → Add-ons → whisper.cpp bridge → Install.** Nothing downloads;
   it just enables the integration.
2. **Restart Tusk's Tomes** so the bridge loads.
3. **Settings → whisper.cpp bridge** — set two paths:
...
4. The panel then reports what it found. You're looking for
...
[YouTube route](../workflows.md) instead**. Upload the recording as an unlisted
...
[roadmap](../../ROADMAP.md) — it needs an upstream project to ship a
