# ❓ Questions I get asked, and a few I asked myself
...
click-by-click version, that's [walkthrough.md](walkthrough.md) — this is the
...
**2. Add a provider key (one-time).** Go to **Settings** → **API Keys**. Pick a provider (Google Gemini Paid is the easiest start; see [pricing.md](pricing.md) for what to expect), paste your key, click **Test connection**. A green OK badge means the key works; an error message tells you what to fix. Your key is encrypted on disk at `%APPDATA%\tusks-tomes\Config\providers.enc` and is *machine-bound* — copying that file to another computer won't decrypt. Recommended starter combo: **Google Gemini — Paid** for everything (see [recommended-settings.md](recommended-settings.md)).
...
**11. Review + export.** The full chronicle, the condensed recap, and the extras render side-by-side on screen. Click **Export → .docx** for a Word document or **Export → .md** for a Markdown file. The exported file lives in your platform-standard Downloads folder by default. If you have a [Tusk's Vault](vault.md) pair configured, the chronicle also lands automatically in your campaign archive.
...
Yes. The software itself is MIT-licensed and free forever — no subscription, no account, no telemetry, no paywall. The only money you might spend is on a cloud LLM API key — Gemini, Claude, and OpenAI are all pay-as-you-go, and Tomes is engineered to keep that bill small (prefix caching, per-tier chunk sizing, Smart Budget routing). A 3-hour session typically lands ~£1.50–£3 on Gemini, ~£0.05–£0.50 on Claude/OpenAI. You can also run it fully offline with [Ollama](https://ollama.com), [LM Studio](https://lmstudio.ai), or Unsloth via the [Local LLMs add-on](add-ons/local-llm.md) — in which case it costs nothing to run. See [pricing.md](pricing.md).
...
Short answer: **Google Gemini (paid)** for the cheapest cloud option overall, **Anthropic Claude** for the best narrative prose, **OpenAI** if you've already got an account there, or a **local model via Ollama** (requires the [Local LLMs add-on](add-ons/local-llm.md)) if you want to spend zero dollars and keep everything offline. You can mix and match per pipeline phase — Settings → Model Profiles for cloud, Settings → Hybrid Routing for the per-phase local-vs-cloud picker. For most groups, Gemini Flash for cleanup + Claude Sonnet for narrative is the sweet spot. See [providers.md](providers.md).
...
<details class="docs-section">
<summary><h2>How is this different from Otter.ai, Descript, or NotebookLM?</h2></summary>
<div class="docs-section-body">


Otter and Descript are generic transcription / podcast-editing tools that have no concept of "this is a TTRPG session" — they'll mis-transcribe every fantasy proper noun and they upload your audio to their cloud. NotebookLM is closer in spirit, but it's a generic research tool — you'd have to manually upload a transcript, manually prompt it, and you'd still get back generic prose rather than a chronicle. Tusk's Tomes is **purpose-built for TTRPG groups**, runs Whisper locally, grounds every chronicle in your campaign's own glossary, and is MIT-licensed. See [comparison.md](comparison.md) for the full breakdown.
...
No — Craig is one workflow, not a requirement. The lowest-friction path is the **YouTube `.sbv` workflow** (works without any add-on): if you already upload sessions to YouTube (public, private, or unlisted), download the auto-caption file from YouTube Studio and drop it in. Craig gives you per-speaker attribution throughout the chronicle, which is great if you want it, but it's optional. See [workflows.md](workflows.md).
...
A self-contained optional capability that bolts onto the core app at runtime. The core program does paste-a-transcript chronicling with cloud LLMs without any add-ons. Each add-on adds a specific slice of functionality. Currently published: **Audio Transcription** (Whisper + Craig + direct audio ingest) and **Local LLMs** (Ollama / LM Studio / Unsloth routing + hybrid per-phase). The architecture is designed to host future ones (diarisation, OCR, etc.). Install/uninstall/toggle-off all happen from Settings → Add-ons. See [add-ons/README.md](add-ons/README.md).
...
No. Your chronicles (`Sessions/<campaign>/...`), encrypted keystore, glossary, speaker mappings, model profiles, and uploaded session audio all live **outside the repo** in your platform-standard config and data directories. The in-app updater (Settings → Updates) does a guarded `git pull` + `npm install` and refuses to clobber uncommitted edits. See [configuration.md](configuration.md) for exactly where things live on disk.
...
1. **Turn on Budget mode.** Settings → Hybrid Routing → **Budget mode** button. One click swaps every phase to the provider's fast-tier model (`gemini-2.5-flash`, `claude-haiku-4-5`, `gpt-5-mini`). Cuts a ~£3 session on Gemini Pro to roughly £0.70 on Flash. Quality on the long-form chronicle phase is noticeably weaker than the flagship model — A/B-test a 30-minute slice before committing if quality matters.
2. **Use Smart Budget instead.** Same UI; a different preset. Paid Flash for grounding + audit, Paid Pro for the chronicle (the quality phase), Paid Flash-Lite for condense. If you've also configured an optional free-tier Gemini key, Smart Budget routes Phase 4 extras to it (the only phase that ever uses your free key under v1.1.0 policy). Typical cost ~£1.50 per 3-hour session — roughly half All-Pro, chronicle quality preserved.
3. **Mix-and-match per phase manually.** Same Hybrid Routing UI. Use the cheap model on Phase 1 (Grounding) and Phase 4 (Extras) where the prose quality barely matters, keep the flagship model on Phase 3 (Chronicle) where it matters.

See [providers.md](providers.md) for honest per-provider cost estimates and the architectural cost-reduction work shipping in the current release.
...
Not yet — it's a [roadmap](../ROADMAP.md) item. A shared community Discord for Tusk's Tomes and the companion project [Tusk's Vault](https://github.com/KochiTusker/Tusks-Vault) (`#support`, `#feature-ideas`, `#showcase`, `#dev-talk`) will be spun up once there's enough signalled interest.
