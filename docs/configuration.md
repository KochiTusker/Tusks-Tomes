...
1. **In-app Settings tab.** Encrypted keystore + per-phase model assignments + per-add-on toggle + (when the [Local LLMs add-on](add-ons/local-llm.md) is loaded) local LLM endpoint. The source of truth after first save.
...
| `{configDir}/tusks-tomes/{glossary,speakers,profiles,routing}.json` | UI-edited config | Per-user, outside the repo |
| `{configDir}/tusks-tomes/addons.json` | Per-add-on `configEnabled` toggle (independent of install state) | Per-user, outside the repo |
| `{configDir}/tusks-tomes/local-llm.enabled` | Marker file for the Local LLMs add-on — present iff installed | Per-user, outside the repo |
...
| `<repo>/vendor/python-venv/` | Whisper sidecar (faster-whisper + torch) — only present when Audio Transcription add-on is installed | Local to this clone, gitignored |
...
The keystore being machine-bound means even if `providers.enc` leaked, the keys are unrecoverable on any other machine. See [privacy.md](privacy.md) for the full trust model.
...
| `ANTHROPIC_API_KEY` | Claude key seed |
| `OPENAI_API_KEY` | OpenAI key seed |
...
| `TUSKS_VAULT_DIR` | Override sister-Vault auto-detect path (see [vault.md](vault.md)) |
...
Variables prefixed with `VITE_` or `PAID_` are inlined into the browser bundle by Vite (configured via `envPrefix` in `vite.config.ts`). Plain-named server-only vars (like `ANTHROPIC_API_KEY`) are **never** sent to the browser.

🚨 **Before you set `TUSKS_HOST=0.0.0.0`** — read this:

This exposes Tusk's Tomes to your local network so other devices (a tablet, your partner's laptop, your phone) can open the app at `http://<your-machine-IP>:5173`. Useful at home; **dangerous on hotel / café / airport / coworking Wi-Fi**. Tradeoffs in plain English:

- ✅ Your API keys are **still safe**. The `/api/provider-keys` route is loopback-only regardless of `TUSKS_HOST`. Devices visiting from elsewhere on the LAN get a 403 on that route. Your Gemini / Claude / OpenAI / Unsloth credentials never leave your computer.
...
- ⚠️ **Everything readable IS visible to anyone on the same Wi-Fi.** Transcripts in progress, finished chronicles, your campaign + session metadata — anyone who can reach your IP can browse them.
- ❌ **Don't do this on public Wi-Fi.** Hotel networks, conferences, coffee shops, libraries. The "same Wi-Fi" attacker is real on those networks and they don't need to be technical — pointing a browser at your IP is enough.
...
For the full security picture (keystore machine-binding, what runs over the network, residual risks we cannot fix in code), see [security-quickref.md](security-quickref.md).
...
`node_modules/`, optionally the Python venv (Audio Transcription add-on), optionally the `local-llm.enabled` marker (Local LLMs add-on), `Sessions/`, the encrypted keystore, settings JSONs, `addons.json` (per-add-on toggle state), `.env`, model caches, session manifests — all gitignored.
...
- Gemini paid: 30k / 30k / 60k / 60k / 100k (Phase 1 / 2 / 3 / 4 / 6)
- Gemini free: 15k / 15k / 35k / 35k / 60k (TPM-tight)
- Claude: 20k / 20k / 40k / 40k / 60k
- OpenAI: 15k / 15k / 30k / 30k / 50k
...
The in-app updater lives at **Settings → Updates → Check for updates → Apply update**. One click pulls the latest code from `main`, re-runs `npm install`, and prompts you to restart. Your encrypted keystore, sessions, glossary, lore, and other gitignored state survive untouched.
