# 🎬 Walkthrough — your first session, end to end
...
### 🅱️ I have a YouTube auto-caption file
...
### 🅲️ I have a Craig multitrack recording (best result)
...
**How:** **Settings → Add-ons → Audio Transcription → Install** (one-time setup that downloads the Whisper sidecar — **5-15 minutes** depending on your connection, since `torch` + CUDA wheels are around 1.5 GB). Restart the server when prompted. Then **Upload** tab → drop the Craig `.zip` → edit speaker mappings while Whisper grinds → **Use this transcript for refinement**.

Deep dive: [AddOns.md → Audio Transcription](../AddOns.md#-audio-transcription).
...
**Settings → API Keys** → paste at least one. Paid Google Gemini is the easiest start ([sign-up walkthrough](providers.md#google-gemini--paid-key-required-)). Claude / OpenAI are also pay-as-you-go.
...
If you've also installed [Tusk's Vault](https://github.com/KochiTusker/Tusks-Vault) as a sibling, a **Send to Vault** button pushes a markdown copy into Vault's `Lore/Tomes/` tree. Vault then makes the chronicle searchable via `@`-mention in Discord. See [docs/vault.md](vault.md).
...
- [Features](features.md) — the full feature list, organised by tab.
- [Workflows](workflows.md) — the three workflow paths in more depth.
- [Providers](providers.md) — picking an LLM, getting an API key, the cost story.
- [AddOns](../AddOns.md) — the three optional add-ons.
- [Architecture](../architecture.md) — what happens under the hood, phase by phase.
