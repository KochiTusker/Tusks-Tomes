...
| Setup-help energy | Watch GitHub issues for setup questions and chip in — that's the single biggest unblock for newcomers (a community Discord with a dedicated support channel is on the [roadmap](ROADMAP.md)) |
...
pitfalls) lives in [SETUP.md](SETUP.md). The full system design lives
...
| A new HTTP API route | `server/api/<resource>.ts` | One router per resource. Mount from `server/index.ts` for core routes; from an add-on's `registerRoutes()` for opt-in routes. |
...
| A new add-on (e.g. live transcription, OCR) | `server/addons/registry.ts` (new entry) + `server/api/<routes>.ts` + `docs/add-ons/<slug>.md` | Existing `audio-addon` / `local-llm-addon`. Each declares `isReady` / `install` / `uninstall` / `registerRoutes` and optionally `docSlug`. |
