...
const POOL = ['Zephyrine', 'Bram', 'Coledge', 'Ilta', 'Nuru', 'Sorrel', 'Wynn', 'Tamsin']
...
describe('substitution changes names and nothing else', () => {
  ...
  it('substitutes every occurrence, in both cases', () => {
    const next = { pc1: 'Nuru', pc2: 'Wynn', pc3: 'Tamsin' }
    const out = rotate(SOURCE, CURRENT, next)
    expect(out).toContain("speakers: ['Nuru', 'Wynn', 'Tamsin']")
    expect(out).toContain('[Nuru (Wynn)]')
    expect(out).toContain("aliases['nuru']")
    expect(out).toContain("'Nuru Vale'")
  })

  it('leaves longer words containing a name alone', () => {
    const out = rotate(SOURCE, CURRENT, { pc1: 'Nuru', pc2: 'Wynn', pc3: 'Tamsin' })
