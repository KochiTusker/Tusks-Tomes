...
const POOL = ['Zephyrine', 'Bram', 'Coledge', 'Ilta', 'Leilani', 'Sorrel', 'Wynn', 'Tamsin']
...
describe('substitution changes names and nothing else', () => {
  ...
  it('substitutes every occurrence, in both cases', () => {
    const next = { pc1: 'Leilani', pc2: 'Wynn', pc3: 'Tamsin' }
    const out = rotate(SOURCE, CURRENT, next)
    expect(out).toContain("speakers: ['Leilani', 'Wynn', 'Tamsin']")
    expect(out).toContain('[Leilani (Wynn)]')
    expect(out).toContain("aliases['leilani']")
    expect(out).toContain("'Leilani Vale'")
  })

  it('leaves longer words containing a name alone', () => {
    const out = rotate(SOURCE, CURRENT, { pc1: 'Leilani', pc2: 'Wynn', pc3: 'Tamsin' })
