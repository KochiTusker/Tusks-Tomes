...
> **Maintainer note**: this section is where the maintainers log
...
_(No items logged yet — yours could be the first. Submit via the [feedback form](https://docs.google.com/forms/d/e/1FAIpQLSdxdqOhb1SQvI3fs50gMJv_Cesh2MuxUm95QO2iZia5sFhyyQ/viewform?usp=header).)_

### 🔵 Researching — needs scoping

<!-- Items the maintainers want to do but haven't sized yet. -->
...
<details class="docs-section">
<summary><h2>🗣️ How to suggest a feature</h2></summary>
<div class="docs-section-body">
...
- **Cost optimisation pass** — Budget mode preset in Hybrid Routing + Phase 3/6 prompt-cache extension + (provider, model-tier)-keyed chunk sizes + Flash rate-limit tier wiring. A 3hr session on Pro that cost ~£3 now lands around ~£0.70 on Flash with caching.
...
- **Privacy scrub + in-app self-updater** — Settings → Updates → Apply
...
- **Telemetry / usage analytics.** None, ever. The maintainer learns about usage via the feedback form (and, once it exists, the community Discord), not phone-home.
...
<details class="docs-section">
<summary><h2>🛠️ Developer notes</h2></summary>
<div class="docs-section-body">
...
system, read [architecture.md](architecture.md). The remainder of this
...
<details class="docs-section">
<summary><h2>Security & hardening — prioritised before any public release</h2></summary>
<div class="docs-section-body">


A deep audit against OWASP Top 10 and MITRE ATT&CK was completed. Full
findings on file; the **fix-list** below is what to act on.

### P0 — fix before running on any network you don't fully control

1. **`server/index.ts:495`** — `httpServer.listen(PORT, …)` defaults to
   binding `0.0.0.0`. Add the host argument: `listen(PORT, '127.0.0.1', …)`.
   Update the log line to match. (OWASP A01; MITRE T1133.)
2. **`server/api/providerKeys.ts`** — `GET /api/provider-keys` returns
   decrypted Gemini/Claude/OpenAI keys with no auth. Gate behind
   `req.ip === '127.0.0.1' || req.ip === '::1'` as defence-in-depth
   even after P0-1. (OWASP A07; MITRE T1552.001.)
3. **Path traversal in session id params** — `server/api/sessions.ts:40,
   116`, `server/api/upload.ts:108, 132, 146`,
   `server/api/transcribe.ts:11, 21`. Validate `:id` against a UUID
   regex (`/^[A-Za-z0-9_-]{8,64}$/`) at every router that joins
   `sessionId` into a filesystem path. (OWASP A01; MITRE T1485.)
4. **SSRF in `/api/local-llm/probe`** — `server/api/probe.ts:21` and
   `server/localProbe/runner.ts:94` accept an arbitrary `baseUrl` and
   `fetch` it server-side, returning the response body in errors.
   Reuse `validateLocalBaseUrl` (`server/index.ts:131`) which is already
   applied correctly to `/api/local/list-models` and
   `/api/local/generate`. (OWASP A10.)
5. **DNS-rebinding** — `Host` header is never validated. Add a small
   middleware that 403s anything outside `localhost`, `127.0.0.1`,
   `[::1]` (plus the declared port). (OWASP A05.)

### P1 — before next public release

6. **Browser-side SDKs hold plaintext keys.**
   `src/lib/providers/claude.ts:104` and
   `src/lib/providers/openai.ts:77` initialise with
   `dangerouslyAllowBrowser: true`. Any XSS exfils every key. Long-term
   fix: move every cloud LLM call server-side and drop the keys from
   the browser entirely. (OWASP A02 / A07.)
7. **Drop `PAID_` from `envPrefix` in `vite.config.ts:10`.** Vite
   inlines `PAID_GEMINI_API_KEY` into the production bundle, which is
   the wrong place for a paid-tier key now that the keystore exists.
   Keep `VITE_` only; remove the seeded read for `PAID_` from the
   keystore boot path.
8. **Security headers.** Add `helmet` (or hand-rolled
   `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`,
   a minimal CSP scoped to the SPA's actual fetch surface).
9. **Chronicle save belt-and-braces.** `server/api/chronicle.ts`'s
   sanitiser is safe in practice but add a final
   `path.resolve(absPath).startsWith(REPO_ROOT + sep + 'Sessions' + sep)`
   check before writing.

### P2 — hygiene

10. **Keystore passphrase layer.** `server/crypto/keyStore.ts` derives
    its KEK from machine identity only; the author explicitly labels
    this obfuscation. Offer an optional user passphrase on top, or
    delegate to the OS keychain (DPAPI / Keychain / libsecret).
11. **Replace unmaintained `pdf-parse@1.1.1`** with `pdfjs-dist` or
    similar maintained library.
12. **Structured logger + redaction.** Today errors `console.error` raw
    stacks; nothing strips key material from error response bodies.
13. **Dependency advisory gate.** Add `npm audit --omit=dev` to CI;
    treat ≥ High as a release blocker.

### Repo cleanliness for publishing

Verified clean as of this writing:

- No committed `.env` / `.env.*` (only `.env.example`, all values blank).
- No hardcoded API keys, tokens, or passwords in tracked files
  (`sk-ant-…`, `AIza…`, `ghp_…`, etc. all return zero hits).
- No personal identifiers in tracked files.
- `vendor/`, `dist/`, `dist-server/`, `node_modules/`, `Sessions/`,
  `sessions/`, `*.log`, `*-err.txt`, `*-out.json` all in `.gitignore`
  and not tracked.
- `server/localProbe/fixtures.ts` carries synthetic D&D names only.

Pre-emptive additions to `.gitignore`: `.idea/`, `.vscode/`,
`coverage/`, `.env.local` (the `.env*.local` glob already covers it but
explicit listing helps future readers).
...
setup; the Craig path lives in [AddOns.md](AddOns.md) +
[docs/add-ons/audio-transcription.md](docs/add-ons/audio-transcription.md).
