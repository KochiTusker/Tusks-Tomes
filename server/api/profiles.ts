...
// Mirror of the KNOWN_MODELS list in server/api/routing.ts — edit BOTH
// when adding a new model (and src/lib/pricing.ts on the browser side).
const KNOWN_MODELS: Record<'gemini' | 'claude' | 'openai' | 'claudeCode' | 'codex', ReadonlySet<string>> = {
  gemini: new Set([
  ...
  // Claude Code accepts model aliases (resolved by the CLI) plus full IDs.
  claudeCode: new Set([
    'sonnet',
    'opus',
    'haiku',
  ...
  // Codex: 'default' defers to the CLI's current default model.
  codex: new Set([
    'default',
    'gpt-5-codex',
    'gpt-5',
...
/** Same tier heuristic as the routing validator — see the note there. A
 *  model profile and a per-phase override must agree on what counts as a
 *  known id, or applying a preset warns in one place but not the other. */
function isKnownProfileModel(provider: keyof typeof KNOWN_MODELS, modelId: string): boolean {
...
const PROVIDER_KEYS = ['gemini', 'claude', 'openai', 'claudeCode', 'codex'] as const
 ...
 *  schema version) → 400. Soft failures (modelId not in KNOWN_MODELS,
 *  often a typo by a user copying a model name from a docs page) →
 *  warning on a 200. The warning is captured in slog so the diagnose
 *  bundle has a trail when a subsequent pipeline call fails. */
function validateProfilesInput(input: unknown): ValidateProfilesResult {
  ...
  if (profilesRaw && typeof profilesRaw === 'object' && !Array.isArray(profilesRaw)) {
    ...
    for (const provider of PROVIDER_KEYS) {
      ...
      for (const field of PROFILE_PHASE_FIELDS) {
        ...
        if (typeof value === 'string' && value.length > 0 && !isKnownProfileModel(provider, value)) {
          const sample = [...KNOWN_MODELS[provider]].slice(0, 4).join(', ')
          warnings.push(
            `profiles.${provider}.${field} '${value}' is not in the known-models list. Pipeline runs may fail at the LLM call if this is a typo. Known IDs include: ${sample}.`,
          )
        }
      }
    }
...
export function profilesRouter(): Router {
  ...
  router.put('/', async (req, res) => {
    try {
      const validated = validateProfilesInput(req.body)
      if (!validated.ok) {
        slog('profiles', {
