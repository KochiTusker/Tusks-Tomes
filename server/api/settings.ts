...
export type Settings = {
   ...
   *    install (or whatever the maintainer's working clone calls origin).
   *  - "dev": a remote literally named `dev`. The maintainer adds it
   *    locally as `git remote add dev https://.../Tusks-Tomes-Dev.git`
   ...
   *  user without dev-repo credentials gets a 404 from `git pull dev`
...
//
//   - The setting persists across restarts (the maintainer doesn't
//     have to re-click "Switch to dev" every reboot).
//   - The session flag does NOT persist (the maintainer must re-type
...
// GitHub's private-repo flag (the dev repo is private; unauthenticated
...
export function settingsRouter(): Router {
  ...
  router.post('/dev-auth', loopbackOnly(), async (req, res) => {
    try {
      ...
      // real security is GitHub's private-repo flag + your cached
