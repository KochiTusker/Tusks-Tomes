...
beforeEach(async () => {
  ...
  vi.doMock('../appData.js', async () => {
    ...
    return {
      ...
      // The validator consults the probe cache, so it must be redirected into
      // the temp dir too. Without this the suite reads whatever the developer
      // last probed on this machine and its results stop being reproducible.
      modelAvailabilityFile: () => path.join(WORK, 'model-availability.json'),
    }
  })
})
...
// The probe is the only component that finds out what a key can actually
// reach. These tests pin the precedence: probe evidence outranks the static
// list in BOTH directions.
describe('routingRouter PUT — probe cache outranks the static list', () => {
  /** Seed the availability cache the validator reads. */
  async function seedProbe(slot: string, probed: Array<Record<string, unknown>>, advertised: string[] = []) {
    await fs.writeFile(
      path.join(WORK, 'model-availability.json'),
      JSON.stringify({ [slot]: { fetchedAt: new Date().toISOString(), advertised, probed } }),
      'utf8',
    )
  }
  ...
  it('accepts a model the probe verified, however unfamiliar the name', async () => {
    // 'quasar-1' names no tier and is on no list. Only the probe vouches for
    // it — which is exactly the forward-compatibility guarantee.
    await seedProbe('gemini', [{ id: 'quasar-1', accessible: true, tier: 'other' }])
    await withRoutingServer(async (baseUrl) => {
      const res = await put(baseUrl, {
        phase3: { target: 'cloud', cloudProvider: 'gemini', geminiTier: 'paid', modelId: 'quasar-1' },
      })
      expect(res.status).toBe(200)
      expect((await res.json()).warnings ?? []).toEqual([])
    })
  ...
  it('warns when the probe proved the model unreachable, even though the heuristic would pass it', async () => {
    // 'gemini-2.5-pro' names a tier, so the static heuristic accepts it and
    // the old validator stayed silent. The probe knows better.
    await seedProbe('gemini', [
      { id: 'gemini-2.5-pro', accessible: false, tier: 'pro', reason: 'Free tier quota: 0 (paid-only)' },
    ])
    await withRoutingServer(async (baseUrl) => {
      const res = await put(baseUrl, {
      ...
      const body = await res.json()
      expect(body.warnings?.length).toBe(1)
      expect(body.warnings[0]).toContain('could not be reached')
      expect(body.warnings[0]).toContain('Free tier quota: 0')
    })
  })

  it("accepts a model on the provider's advertised list even if unprobed", async () => {
    await seedProbe('gemini', [{ id: 'gemini-2.5-flash', accessible: true, tier: 'flash' }], ['nebula-x'])
    await withRoutingServer(async (baseUrl) => {
      const res = await put(baseUrl, {
        phase3: { target: 'cloud', cloudProvider: 'gemini', geminiTier: 'paid', modelId: 'nebula-x' },
      })
      expect((await res.json()).warnings ?? []).toEqual([])
    })
  })

  it("falls back to the static list when the slot has no probe data", async () => {
    // Cache holds the free slot only; the phase routes to paid. Nothing
    // vouches either way, so the static heuristic decides — and a tier-named
    // id still passes, exactly as on a fresh install.
    await seedProbe('geminiFallback', [{ id: 'gemini-2.5-flash', accessible: true, tier: 'flash' }])
    await withRoutingServer(async (baseUrl) => {
      const res = await put(baseUrl, {
        phase3: { target: 'cloud', cloudProvider: 'gemini', geminiTier: 'paid', modelId: 'gemini-7-pro' },
      })
      expect((await res.json()).warnings ?? []).toEqual([])
    })
  })

  it("on tier 'auto', accessible on either key slot is enough", async () => {
    // The pipeline may dispatch to either key when the tier is auto, so a
    // model reachable on only one of them must not warn. Ordering matters:
    // the free slot is checked first and fails, and a naive implementation
    // would report inaccessible before ever reaching the paid slot.
    await fs.writeFile(
      path.join(WORK, 'model-availability.json'),
      JSON.stringify({
        geminiFallback: {
          ...
          advertised: [],
          probed: [{ id: 'gemini-2.5-pro', accessible: false, reason: 'Free tier quota: 0 (paid-only)' }],
        },
        gemini: {
          fetchedAt: new Date().toISOString(),
          advertised: [],
          probed: [{ id: 'gemini-2.5-pro', accessible: true }],
        },
      }),
      'utf8',
    )
    await withRoutingServer(async (baseUrl) => {
      const res = await fetch(baseUrl, {
        ...
        body: JSON.stringify({
          ...
          geminiTier: 'auto',
          perPhase: {
            phase3: { target: 'cloud', cloudProvider: 'gemini', modelId: 'gemini-2.5-pro' },
          },
        }),
      })
      expect((await res.json()).warnings ?? []).toEqual([])
