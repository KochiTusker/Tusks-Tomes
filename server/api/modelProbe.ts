...
/** The Pro/Flash filter narrows the probe set to "models the chronicle
 *  pipeline could plausibly use." We deliberately skip embedding, tts,
 *  imagen, and other specialty families — probing those would consume
 *  free-tier quota with no payoff for the picker. */
function shouldProbeModel(m: GeminiListedModel): boolean {
  const id = (m.name ?? '').replace(/^models\//, '')
  if (!id) return false
  if (!(m.supportedGenerationMethods ?? []).includes('generateContent')) return false
  const tier = classifyTier(id)
  return tier === 'pro' || tier === 'flash'
}
...
async function probeOneGeminiModel(
  ...
  try {
    ...
    return { id, accessible: false, reason: classifyProbeFailure(res.status, body), latencyMs }
  } catch (err) {
    return {
      id,
      accessible: false,
 ...
 *
 * Probe budget: at most 30 models tested with a 1-token generateContent.
 * For free-tier keys this consumes ~30 of the per-day RPD; we cap to keep
 * the cost bounded if Google ever ships a much larger model catalog.
 */
