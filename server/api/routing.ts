...
import { migrateRouting } from '../lib/legacyProviders.js'
import {
  acceptModel,
...
// Model recognition lives in server/lib/knownModels.ts, shared with
// profiles.ts. It consults the probe cache first and only falls back to a
// static list when no probe has run for the relevant key slot — so a model
// the probe verified against the user's own key never warns, however new it
// is, and a model the probe proved unreachable warns even if it happens to
// be on the static list.
 ...
 *  400. Soft failures (a modelId the probe cache can't vouch for) return as
 *  warnings on a 200. Reads stay lenient (sanitize) so older on-disk shapes
 *  still load — only the writer enforces the new contract.
 *
 *  `cache` is the probe availability cache, passed in rather than read here
 *  so one PUT validates every phase against a single read. */
function validateInput(input: unknown, cache: AvailabilityCache): ValidateResult {
        ...
        if (typeof e.modelId === 'string' && e.modelId.length > 0) {
          ...
          const effectiveProvider: CloudProvider | null = isCloudProvider(e.cloudProvider)
            ...
            : isCloudProvider(raw.lastSelectedProvider)
              ...
              : null
          // Same inheritance for the tier — it decides which key slot's probe
          // result speaks for this phase.
          const effectiveTier: GeminiTier | undefined = isGeminiTier(e.geminiTier)
            ? e.geminiTier
            : isGeminiTier(raw.geminiTier)
              ? raw.geminiTier
              : undefined
          if (effectiveProvider) {
            const verdict = acceptModel(effectiveProvider, effectiveTier, e.modelId, cache)
            if (!verdict.ok) {
              warnings.push(
                warningFor(`perPhase.${k}.modelId`, effectiveProvider, e.modelId, verdict),
              )
            }
          }
        }
...
async function loadOrSeed(): Promise<RoutingDocument> {
  ...
  // Config written before the direct Anthropic and OpenAI keys were retired
  // names providers that no longer exist. Rewrite it to the OpenRouter
  // equivalent — same model, different key — BEFORE sanitize(), which would
  // otherwise drop the unrecognised entries and silently reset those phases.
  const { doc, notices } = migrateRouting(existing)
  if (notices.length > 0) {
    slog('server', {
      event: 'routing_migrated_off_retired_providers',
      phases: notices.map((n) => `${n.phase}:${n.from}→openrouter`).join(','),
    })
...
export function routingRouter(): Router {
  ...
  router.put('/', async (req, res) => {
    try {
      const validated = validateInput(req.body, await loadAvailabilityCache())
      if (!validated.ok) {
        slog('routing', {
