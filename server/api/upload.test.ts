...
// These tests POST to the REAL upload router, which runs the real
// extractor and writes a session directory. Session storage resolves to
// a machine-global path (env-paths), NOT something repo-relative — so
// without this override every `npm test` run dropped a junk session into
// the developer's actual Sessions tab. sessionsRoot() reads the env var
// per call, so setting it here is enough.
let sessionsTmpDir: string | undefined
let priorSessionsDir: string | undefined

beforeAll(async () => {
  priorSessionsDir = process.env.TUSKS_SESSIONS_DIR
  // mkdtemp (not a fixed path): unique per run, and the dangerous-fs.rm
  // audit only accepts an rm target that traces to mkdtemp.
  sessionsTmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tusks-tomes-test-sessions-'))
  process.env.TUSKS_SESSIONS_DIR = sessionsTmpDir
})

afterAll(async () => {
  if (priorSessionsDir === undefined) delete process.env.TUSKS_SESSIONS_DIR
  else process.env.TUSKS_SESSIONS_DIR = priorSessionsDir
  if (sessionsTmpDir) {
    await fs.rm(sessionsTmpDir, { recursive: true, force: true }).catch(() => undefined)
  }
})
