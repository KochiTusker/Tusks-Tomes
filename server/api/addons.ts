...
export function addonsRouter(): Router {
  ...
  router.get('/', async (_req, res) => {
    try {
      ...
      // Only installable add-ons appear in the UI list. Builtins mount
      // unconditionally and configure themselves through their own surfaces
      // (connection panels, the lore-source switch), so an install/uninstall
      // row for them would be ceremony around a marker file.
      const installable = ADDON_REGISTRY.filter(
        (a): a is Extract<typeof a, { kind: 'install' }> => a.kind === 'install',
      )
      const addons = await Promise.all(
        installable.map(async (addon) => {
          let ready = false
  ...
  router.patch('/:name', async (req, res) => {
    ...
    if (addon.kind !== 'install') {
      return res.status(400).json({
        error: `"${addon.displayName}" is part of the app — there is nothing to toggle.`,
      })
    }
  ...
  router.post('/:name/install', async (req, res) => {
    ...
    if (addon.kind !== 'install') {
      return res.status(400).json({
        error: `"${addon.displayName}" is part of the app — there is nothing to install.`,
      })
    }
  ...
  router.delete('/:name', async (req, res) => {
    ...
    if (addon.kind !== 'install') {
      return res.status(400).json({
        error: `"${addon.displayName}" is part of the app — there is nothing to uninstall.`,
      })
    }
    try {
