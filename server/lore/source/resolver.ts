...
export const OBSIDIAN_MARKER_FILENAME = 'obsidian-vault.enabled'
...
export function obsidianMarkerPath(): string {
  return path.join(configDir(), OBSIDIAN_MARKER_FILENAME)
}
...
/** Returns the active Obsidian source when the add-on is installed (marker
 *  present), config is enabled, and the vault path is a readable directory;
 *  otherwise null (→ caller falls back to Tusks-Lore). */
export async function resolveObsidianSource(): Promise<ActiveObsidianSource | null> {
  if (!(await markerPresent())) return null
